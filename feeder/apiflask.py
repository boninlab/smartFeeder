...
class ApiFlask(FlaskView):
  ...
  def status(self):
    global g_rd
    global mainT
    fobj = {"devices": {}, "params": {}, "hoppers": {}, "tasks": {}}
    ...
    tasklist = []
    for tid in mainT.tasks:
      task = mainT.tasks[tid]
      tasklist.append(
          {'id': tid, 'type': task.taskType, 'state': task.state})
    fobj['tasks'] = tasklist
...
def runApiFlask(mainTask):
  global mainT
  mainT = mainTask
  ApiFlask.register(app, route_base='/')
  app.run(host='0.0.0.0', port=3100)  # debug는 main thread 에서만 작동
